...
- **Public API for Addons — new methods:**
  - `LandClaimAPI.getAllClaimProfiles()` — every loaded claim profile, for global iteration (tax, leaderboards, server-wide searches).
  - `LandClaimAPI.getClaimById(UUID)` — look up a claim by its profile id.
  - `LandClaimAPI.transferClaim(Player actor, UUID profileId, UUID newOwnerId)` — programmatically reassign ownership. The actor must be the new owner (self-transfer for marketplaces) or have `landclaim.admin`. Used by the official [LandClaimPlugin-Economy](https://github.com/synkfr/LandClaimPlugin-Economy) marketplace and auction flows.
  - `LandClaimAPI.unclaimAll(Player actor, UUID profileId)` — wipe every chunk of a profile in one transaction. The actor must be `null` (system call, e.g. tax auto-unclaim) or have `landclaim.admin`.
  - `LandClaimAPIImpl` now also publishes `INVALIDATE_PROFILE` on Redis sync for both `transferClaimProfile` branches so multi-server networks see marketplace sales on every node immediately.
